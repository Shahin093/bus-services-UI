...
const BusModal = ({ bookingBus, setBookingBus }) => {
  ...
  useEffect(() => {
    fetch(
      `http://localhost:5000/api/v1/busCollection/slots?slot=${selectedSlot}&from=${selectedFrom}&to=${selectedTo}&bus_name=${bookingBus?.bus_name}&&dates=${dates}`
    )
      .then((res) => res.json())
      .then((data) =>
        data?.data?.map((sl) => {
      ...
      <div className="modal modal-bottom sm:modal-middle">
        <div className="modal-box ">
            ...
            <div className=" card w-full bg-gray-200 shadow-xl">
              <div className="card-body">
                <form action="/dashboard" onSubmit={handleSubmit(onSubmit)}>
                  <h2 className="mx-auto text-blue-600 text-3xl font-bold">
                    Welcome to{" "}
                    <span className="underline decoration-pink-500 decoration-[10px] text-yellow-500">
                      {bookingBus?.bus_name}
                    </span>{" "}
                    Bus
                  </h2>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
                    ...
                    <div className="form-control w-full max-w-xs">
                      ...
                      <select
                        ...
                        // })}
                      >
                        <option className="bg-orange-500 font-bold" value="">
                          Select Destination
                        </option>
                        {bookingBus?.district_to.map((dt, index) => (
                          <option
                            key={index}
                      ...
                      {bo?.map((b) =>
                        co.includes(b) ? (
                          <span>
                            <img
                              width={"40px"}
                              src="https://image.similarpng.com/very-thumbnail/2020/09/Right-mark-icon-on-transparent-background-PNG.png"
                              alt=""
                            />
                          </span>
                        ) : (
                      ...
                      <label className="label">
                        <span className="label-text mx-auto">
                          ...
                          selectedSlot.split(":", 1)[0] ? (
                            ...
                            <div>
                              {" "}
                              <h2 className="label-text-alt text-green-500">
                                Everything is Okay
                              </h2>
                              <img
                                width={"80px"}
                    ...
                    <input
                      disabled={
                        parseInt(showTime.split(":", 1)[0]) >=
                          parseInt(selectedSlot.split(":", 1)[0]) ||
                        seatCount > 50 - seat ||
                        localStorage.setItem("tickets1", JSON.stringify(cart))
                          ?.length === 0
                      }
                      className="p-5 bg-yellow-500 submit-button"
                      type="submit"
                      value="Booking Now"
                    />
                    {dataValid && (
          ...
          <div className="modal-action">
            <label htmlFor="my-modal-6" className="bg-yellow-500">
              Close
            </label>
          </div>
        </div>
